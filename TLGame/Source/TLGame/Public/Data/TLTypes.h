...
public:
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Conditions") FGameplayTagContainer RequiredConditions;  // Требования для показа этого узла (если используем рандом) Например: Враг должен быть Angry1
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Dialogue") TArray<FPlayerResponse> PlayerResponses;  // Список ответов Игрока
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Dialogue") FName RowID;
    // Картинка эмоции (опционально, если хотим хардкод, но лучше через теги) UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Visuals")
